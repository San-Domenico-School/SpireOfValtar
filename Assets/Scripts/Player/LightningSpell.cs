...
public class LightningSpell : MonoBehaviour
{
  ...
  [SerializeField] private GameObject impactVfxPrefab; // drag your particle prefab here
  [SerializeField] private float impactVfxLifetime = 2f; // safety cleanup
  ...
  private void FireRaycast()
  {
    ...
    if (Physics.Raycast(origin, direction, out RaycastHit hit, range))
    {
      ...
      // Show the impact (collision) particle right here
  ...
  private void SpawnImpactVfx(Vector3 position, Vector3 normal)
  {
    ...
    // Rotate particle to face outward from surface
    Quaternion rotation = Quaternion.LookRotation(normal, Vector3.up);

    GameObject vfx = Instantiate(impactVfxPrefab, position, rotation);

    // Safety cleanup (unless your particle auto-destroys)
